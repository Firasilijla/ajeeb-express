...
use App\Models\WhyUsTranslation;
...
class WhyUsController extends Controller
{
    public function index(Request $request)
    {
                    ...
                    return '
                        <td class="text-right pr-0">
                            <div class="btn-group btn-group-sm">
                                <a href="javascript:void(0)" data-id="' . $row->id . '" class="btn btn-icon btn-light btn-hover-danger btn-sm delete-whyus">
                                ...
                                <a data-toggle="modal" data-target="#edit-exampleModalSizeXl" href="javascript:void(0)" class="btn btn-icon btn-light btn-hover-success btn-sm edit-whyus" data-id="' . $row->id . '">
        ...
        return view('Admin.WhyUs.index');  // عرض الصفحة الرئيسية
    }
    ...
    public function store(WhyUsRequest $request)
    {
        ...
        try {
            ...
            if ($request->has("whyusImage")) {
                $file = $request->file('whyusImage');
                $filename = $file->hashName();
                $imagePath = 'images/WhyUs/' . $filename; // تخزين الصورة في المسار الصحيح
                $file->storeAs('public/images/WhyUs', $filename);  // تخزين الصورة في مجلد "public/images/WhyUs"
            }

            // تخزين البيانات في جدول WhyUs باستخدام create()
            $whyUs = WhyUs::create([
                'image' => $imagePath,
                'orders_delivered' => $request->orders_delivered,  // إضافة عدد الطلبات
                'satisfied_clients' => $request->satisfied_clients // إضافة عدد العملاء الراضين
            ]);

            // إضافة الترجمات
            $whyUs->translations()->create([
                'language' => 'en',
    ...
    public function edit($id)
    {
        ...
        $translations = $whyUs->translations;  // الحصول على الترجمات إذا كانت موجودة في جدول منفصل
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'image' => $whyUs->image ?? 'default.jpg', // تأكد من وجود الصورة
            ]
        ]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $whyUs = WhyUs::findOrFail($id); // البحث عن السجل باستخدام الـ id

            // تحديث الصورة إذا تم رفع صورة جديدة
            $imagePath = $whyUs->image;  // المسار القديم للصورة

            if ($request->hasFile('whyusImage')) {
                // مسار الحذف للصورة القديمة (من المجلد public/images/WhyUs)
                $oldImagePath = 'public/' . $whyUs->image;
                ...
                $imagePath = 'images/WhyUs/' . $filename;  // مسار الصورة الجديد

                // تخزين الصورة في المسار الصحيح
                $file->storeAs('public/images/WhyUs', $filename);
            }

            // تحديث سجل why_us مع المسار الجديد للصورة
            $whyUs->update([
                'image' => $imagePath,
                'orders_delivered' => $request->orders_delivered,
                'satisfied_clients' => $request->satisfied_clients,
            ]);

            // تحديث الترجمات
            $whyUs->translations()->where('language', 'en')->update([
                'title' => $request->title_en,
                'description' => $request->description_en
            ]);

            $whyUs->translations()->where('language', 'ar')->update([
                'title' => $request->title_ar,
                'description' => $request->description_ar
            ]);
    ...
    public function destroy($id)
    {
        try {
            ...
            if (file_exists(public_path('storage/images/WhyUs/' . $whyUs->image))) {
                unlink(public_path('storage/images/WhyUs/' . $whyUs->image));
            }
