...
class ServiceController extends Controller
{
    public function index(Request $request)
    {
                    ...
                    return '
                        <td class="text-right pr-0">
                            <div class="btn-group btn-group-sm">
                                <a href="javascript:void(0)" data-id="' . $row->id . '" class="btn btn-icon btn-light btn-hover-danger btn-sm delete-service">
                                ...
                                <a data-toggle="modal" data-target="#edit-exampleModalSizeXl" href="javascript:void(0)" class="btn btn-icon btn-light btn-hover-success btn-sm edit-service" data-id="' . $row->id . '">
        ...
        return view('Admin.Service.index');  // عرض الصفحة الرئيسية
    }
    ...
    public function store(ServiceRequest $request)
    {
        ...
        try {
            ...
            if ($request->has("serviceImage")) {
                ...
                $imagePath = 'images/Service/' . $filename; // تخزين الصورة في المسار الصحيح
                $file->storeAs('public/images/Service', $filename);  // تخزين الصورة في مجلد "public/images/WhyUs"
            }

            // تخزين البيانات في جدول WhyUs باستخدام create()
            $whyUs = Service::create([
                'image' => $imagePath,
    ...
    public function edit($id)
    {
        ...
        $translations = $service->translations;  // الحصول على الترجمات إذا كانت موجودة في جدول منفصل
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'image' => $service->image ?? 'default.jpg', // تأكد من وجود الصورة
            ]
        ]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $service = service::findOrFail($id); // البحث عن السجل باستخدام الـ id

            // تحديث الصورة إذا تم رفع صورة جديدة
            $imagePath = $service->image;  // المسار القديم للصورة

            if ($request->hasFile('serviceImage')) {
                // مسار الحذف للصورة القديمة (من المجلد public/images/WhyUs)
                $oldImagePath = 'public/' . $service->image;
                ...
                $imagePath = 'images/Service/' . $filename;  // مسار الصورة الجديد

                // تخزين الصورة في المسار الصحيح
                $file->storeAs('public/images/Service', $filename);
            }
            ...
            // تحديث الترجمات
            $service->translations()->where('language', 'en')->update([
                'title' => $request->title_en,
                'description' => $request->description_en
            ]);

            $service->translations()->where('language', 'ar')->update([
                'title' => $request->title_ar,
                'description' => $request->description_ar
            ]);
    ...
    public function destroy($id)
    {
        try {
            ...
            if (file_exists(public_path('storage/images/Service/' . $Service->image))) {
                unlink(public_path('storage/images/Service/' . $Service->image));
            }
