...
<div class="d-flex align-items-center " id="kt_subheader_search">
    <span class="text-dark-50 font-weight-bold" id="kt_subheader_total">WhatWeDo </span>
</div>
<div class="subheader-separator subheader-separator-ver mt- mb-2 ml-2 mr-2 bg-gray-200"> </div>
        ...
        <div id="" class="">
            <div class="row">
                <div class="col-sm-12 table-container">
                    <table class="table table-separate table-head-custom table-checkable dataTable no-footer dtr-inline collapsed" id="WhatWeDos" role="grid" aria-describedby="kt_datatable_info" style="width: 957px;">
                        <thead>
                            <th><span>#</span></th>
...
<!-- ------------------- modal --------------  -->
<!-- ------------------- modal -------------- -->
<div class="modal fade  {{ \App\Helpers\Helper::getTextDirection() }} stor-WhatWeDo-modal" id="exampleModalSizeXl2" tabindex="-1" role="dialog" aria-labelledby="exampleModalSizeXl" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-title text-white " id="exampleModalLabel"> Add WhatWeDo </h5>
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="image-input image-input-outline" id="kt_image_4">
                                            <div class="image-input-wrapper" id="WhatWeDoImage" style="background-image: url('<?php echo asset('assets/media/bg/bg-9.jpg') ?>'); center; background-size: cover; height: 150px; width: 150px;"></div>
                                            <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="إختر الصورة">
                                                ...
                                                <input type="file" name="WhatWeDoImage" accept=".png, .jpg, .jpeg" />
                                            </label>
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="store-WhatWeDo" class="btn btn-primary font-weight-bold store-WhatWeDo">اضافة</button>
            </div>
        </div>
    </div>
</div>
<div class="modal fade {{ \App\Helpers\Helper::getTextDirection() }} edit-WhatWeDo-modal" id="edit-exampleModalSizeXl" tabindex="-1" role="dialog" aria-labelledby="exampleModalSizeXl" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title text-white" id="exampleModalLabel"> Edit WhatWeDo </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i aria-hidden="true" class="ki ki-close"></i>
                </button>
            </div>
            <div class="modal-body" data-scroll="true" data-height="500">
                <form class="form" id="myFormupdate" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method("PUT")
                    <input type="hidden" name="WhatWeDo_id" id="WhatWeDo_id">
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="image-input image-input-outline" id="kt_image_3">
                                            <div class="image-input-wrapper" id="edit_WhatWeDoImage" style="background-image: url('<?php asset('assets/media/bg/bg-9.jpg') ?>'); center; background-size: cover; height: 150px; width: 150px;"></div>
                                            <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="إختر الصورة">
                                                <i class="fa fa-pen icon-sm text-muted"></i>
                                                <input type="file" name="WhatWeDoImage" accept=".png, .jpg, .jpeg" />
                                            </label>
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="update-WhatWeDo" class="btn btn-success font-weight-bold ">تعديل</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(function() {
        var table = $('#WhatWeDos').DataTable({
            ...
            ajax: {
                ...
                url: "{{ route('admin.WhatWeDos.index') }}", // Ensure controller route is correct
    ...
    $('body').on('click', '#store-WhatWeDo', function() {
        let myForm = document.getElementById('myForm');
        let formData = new FormData($('#myForm')[0]);
        $.ajax({
            type: "POST",
            url: "{{ route('admin.WhatWeDos.store') }}", // تأكد من تغيير الرابط إلى المسار الصحيح
            ...
            success: function(res) {
                if (res.status == 1) {
                    var oTable = $('#WhatWeDos').dataTable(); // تأكد من تغيير اسم الجدول إذا لزم الأمر
                    oTable.fnDraw(false);
                    $('#myForm')[0].reset();
                    ...
                    $(".stor-WhatWeDo-modal").modal('toggle');
                    $("#title_ar").text("");
                    $("#title_en").text("");
                    $("#description_ar").text("");
                    $("#description_en").text("");
    ...
    $('body').on('click', '.edit-WhatWeDo', function() {
        var WhatWeDo_id = $(this).data('id'); // الحصول على الـ ID من الـ data-attribute

        $.ajax({
            type: "GET",
            url: "{{ route('admin.WhatWeDos.edit', ':WhatWeDo_id') }}".replace(':WhatWeDo_id', WhatWeDo_id), // تمرير الـ ID بشكل صحيح في الـ URL
            data: {
                "_token": "{{ csrf_token() }}",
                'id': WhatWeDo_id
            },
            ...
            success: function(res) {
                ...
                if (res.success) {
                    ...
                    $(".edit-WhatWeDo-modal").modal('toggle'); // فتح المودال للتعديل

                    // تعبئة الحقول في الفورم بالبيانات التي تم جلبها
                    $("#WhatWeDo_id").val(res.data.id);
                    //  
                    ...
                    $("#edit_WhatWeDoImage").css("background-image", "url(" + update_avatar_url + ")");
    ...
    $('body').on('click', '#update-WhatWeDo', function() {
        ...
        $.ajax({
            ...
            url: "{{ route('admin.WhatWeDos.update', ':WhatWeDo_id') }}".replace(':WhatWeDo_id', WhatWeDo_id),
            enctype: "multipart/form-data",
            data: formData,
            processData: false,
            contentType: false,
            success: function(res) {
                if (res.success) {
                    $(".edit-WhatWeDo-modal").modal('toggle');
                    var oTable = $('#WhatWeDos').DataTable();
    ...
    $('body').on('click', '.delete-WhatWeDo', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    url: "/admin/WhatWeDos/" + id, // المسار الذي يحدد العنصر بناءً على الـ ID
                    data: {
