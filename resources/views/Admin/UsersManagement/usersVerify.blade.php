                ...
                <div class="col-sm-12">
                    <table class="table table-separate table-head-custom table-checkable dataTable no-footer dtr-inline collapsed" id="admins" role="grid" aria-describedby="kt_datatable_info" style="width: 957px;">
                        <thead style="background-color: rgba(27, 160, 143, 0.544);color:black">
                            ...
                            <th><span style="color:black;font-weight: bolder;font-family: Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif;">صورة الهويه</span></th>
                            ...
                            <th><span style="color:black;font-weight: bolder;font-family: Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif; ;text-align: center;">حالة التوثيق</span> </th>
                        </thead>
...
<script>
    $(function() {
        ...
        var table = $('#admins').DataTable({
            ...
            "ajax": {
                ...
                "url": "{{ route('admin.admin.indexVerify') }}",
                "type": "GET"
    ...
    $('body').on('click', '.change_status', function() {
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('admin.admin.acceptsVerify') }}",
            data: {
                "_token": "{{ csrf_token() }}",
                'id': id,
                'status': status,
            ...
            success: function(res) {
                ...
                if (res.status === '1') {
                    if (status == 1) {
                        Swal.fire({
                            title: 'تم  توثيق حساب  المستخدم ',
                            text: 'تمت عمليه  توثيق حساب المستخدم ب نجاح',
                            confirmButtonText: 'تم  ',
                            icon: 'success',
                        });
                    } else {
                        Swal.fire({
                            title: 'تم  تعطيل  توثيق حساب ',
                            text: 'تمت عمليه تعطيل   توثيق حساب المستخدم ب نجاح',
                            confirmButtonText: 'تم  ',
                            icon: 'error',
                        });
                    }
                ...
                if (res.status === '0') {
                    Swal.fire({
                        title: ' فشلت عمليه تعديل التوثيق  ',
                        text: 'فشلت عمليه  تعديل التوثيق ',
                        confirmButtonText: 'تم  ',
                        icon: 'warning',
                    });
                }
