                ...
                <div class="card-body p-5">
                    ...
                    <form action="#" method="POST" id="register_form">
                        ...
                        <div class="text-center text-secondary">
                            <div>Already Have account? <a href="/" class="text-decoration-none">Login Here</a></div>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(function() {
        $("#register_form").submit(function(e) {
            ...
            $.ajax({
                url: "{{route('User.auth.register')}}",
                method: "post",
                data: $(this).serialize(),
                dataType: 'json',
                success: function(res) {
                    if(res.status==200){
                        $("#show_success_alert").html(showMessage('success',res.msg));
                ...
                error: function(reject) {
                    showStateMessage('name','');
                    showStateMessage('password','');
                    showStateMessage('cpassword','');
